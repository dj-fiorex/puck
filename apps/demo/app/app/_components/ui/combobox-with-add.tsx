...
export type ComboboxWithAddOptions = {
  value: string;
  label: string;
};

interface ComboboxWithAddProps {
  options: ComboboxWithAddOptions[];
  selected: ComboboxWithAddOptions["value"];
  className?: string;
  placeholder?: string;
  disalbed?: boolean;
  onChange: (option: ComboboxWithAddOptions) => void;
  onCreate?: (label: ComboboxWithAddOptions["label"]) => void;
}
...
export function ComboboxWithAdd({
  ...
  function handleSelect(option: ComboboxWithAddOptions) {
    if (onChange) {
      onChange(option);
      setOpen(false);
      setQuery("");
              ...
              {options.map((option, index) => (
                <CommandItem
                  ...
                  className={cn(
                    ...
                    "hover:!bg-blue-200 focus:!bg-blue-200 aria-selected:bg-transparent",
                  )}
                >
